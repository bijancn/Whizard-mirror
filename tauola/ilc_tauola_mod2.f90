...
module ilc_tauola_mod2
  ...
  integer                      ::  np1
  integer                      ::  np2
  double precision, dimension(4) :: q1
  double precision, dimension(4) :: q2
  double precision, dimension(4) :: p1
  double precision, dimension(4) :: p2
  double precision, dimension(4) :: p3
...
contains
  ...
  subroutine fill_pyjets_spin_data
    ...
    do ip = 1, n_doc_lines
       ...
       if ( pyjets_spin_data(ip)%pid == 25 .and. pyjets_spin_data(ip)%n_daughter == 2 ) then
          idau1=pyjets_spin_data(ip)%index_daughter(1)
          idau2=pyjets_spin_data(ip)%index_daughter(2)
          if( abs(pyjets_spin_data(idau1)%pid)==15 .and. (pyjets_spin_data(idau1)%pid+pyjets_spin_data(idau2)%pid)==0 ) then
             if(pyr(0) < 0.5) then
       ...
       else if ( pyjets_spin_data(ip)%pid == 23 .and. pyjets_spin_data(ip)%n_daughter == 2 ) then
          idau1=pyjets_spin_data(ip)%index_daughter(1)
          idau2=pyjets_spin_data(ip)%index_daughter(2)
          if( abs(pyjets_spin_data(idau1)%pid)==15 .and. (pyjets_spin_data(idau1)%pid+pyjets_spin_data(idau2)%pid)==0 ) then
  ...
  subroutine ilc_tauola_pytaud(itau,iorig,kforig,ndecay)
    ...
    integer, intent(in)  :: iorig  ! Line number where the mother is stored. =0 if the mother is not stored
    ...
    integer :: ip
    print *, 'ilc_tauola_pytaud' !!! Debugging
    call pylist(2)
    print *, 'itau =    ', itau !!! Debugging
    ! TODO: (bcn 2016-03-11) this should only be called once per event (not per tau)
    ...
    call do_dexay (itau, p_dexay)
    ndecay = nproducts
    call pyhepc(2) ! translates the current HEPEVT record into the PYJETS one, while leaving the original HEPEVT one unaffected
  end subroutine ilc_tauola_pytaud


  subroutine do_dexay (itau, p_dexay)
    integer, intent(in) :: itau
    double precision, dimension(5), intent(in) :: p_dexay
    integer   :: i
    print *, 'do_dexay' !!! Debugging
    ...
    print *, 'nproducts =    ', nproducts !!! Debugging
