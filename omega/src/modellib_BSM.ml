(* $Id: modellib_BSM.ml 7305 2015-10-03 12:32:42Z msekulla $
...
let rcs_file = RCS.parse "Modellib_BSM" ["BSM Models"]
    { RCS.revision = "$Revision: 7305 $";
      RCS.date = "$Date: 2015-10-03 14:32:42 +0200 (Sat, 03 Oct 2015) $";
...
(* Optionally, ask for the fudge factor treatment for the widths of
   charged particles.  Currently, this only applies to $W^\pm$ and top. *)
    ...
    let charge = function
      ...
      | O f ->
          begin match f with
          | H | Rsigma | Phi0 | Rphin | Rphisn | Rf | Rtn ->  0//1
          | Phip | Rphip | Rtp ->  1//1
          | Phim | Rphim | Rtm -> -1//1
          | Rphipp | Rtpp ->  2//1
          | Rphimm | Rtmm -> -2//1
...
(* Optionally, ask for the fudge factor treatment for the widths of
   charged particles.  Currently, this only applies to $W^\pm$ and top. *)
    ...
    let charge = function
      ...
      | O f ->
          begin match f with
          | H | Rsigma | Phi0 | Rphin | Rphisn | Rf | Rff | Rfv | Rfphi | Rtn ->  0//1
          | Phip | Rphip | Rtp ->  1//1
          | Phim | Rphim | Rtm -> -1//1
          | Rphipp | Rtpp ->  2//1
          | Rphimm | Rtmm -> -2//1
