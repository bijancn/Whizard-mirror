...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\preprintno{First version: November 1997}
\title{%
 \GML:\\[.5\baselineskip]
...
\author{%  
 ...
 {\it with contributions from} \\
 Thorsten Ohl$^b$,%
 \email{ohl@physik.uni-wuerzburg.de}
 J\"urgen Reuter$^c$%
 \email{juergen.reuter@desy.de}
}
\address{%
${}^a$\, Universit\"at Siegen, Theoretische Physik 1,\\
 Walter-Flex-Str. 3, D-57068 Siegen, Germany \\
${}^b$\, Universit\"at W\"urzburg, Theoretische Physik II\\
Emil-Hilb-Weg 22, D-97074 W\"urzburg, Germany \\
${}^c$\, DESY, Theory Group, \\
...
This manual describes the latest version 0.40 incorporated in
\texttt{WHIZARD}. 
}
%
\maketitle
...
\begin{figure}
  \begin{quote}
    ...
    `\end{document}`
  \end{quote}
\caption{\label{fig:document} Schematic structure of a \GML\ document.}
\end{figure}
...
\LaTeX\ document.  On the other hand, \GML\ will invoke \LaTeX\ to
...
output
\begin{quote}
`This is MetaPost, Version 1.802 (kpathsea version 6.1.1)`\\
`[....]`\\
`(./foo.mp`\\
...
`1pt` (this is a "point" which is equal to 1/72.27 inch or 0.3515 mm);
it is usually a good idea to set it explicitly immediately 
...
\begin{quote}
`$ latex foo; whizard-gml foo; latex foo`%$
\end{quote}
and try to view the result
\begin{quote}
`$ xdvi foo`%$
\end{quote}
the \PS\ interpreter inside `xdvi` could possibly crash.  Similarly,
...
self-contained EPS files. Of course, 
...
imaginary circular pen of diameter `0.5bp` (`bp` is a "big point"
which is equal to 1/72 inch or 0.3527 mm). This can be locally reset
...
`gmlcolors.tex` and `gmlextracolors.tex`.  These files can be found in
\texttt{src/gamelan} (or in the corresponding \texttt{build} directory
for the SVN version). They contain a collection
...
\end{quote}
Note that these files need to be copied to your current directory, or
must be present in a known \LaTeX\ path.
...
(i.e. figures that have been generated in a previous \LaTeX\ run of
the same document will be displayed anyhow).
...
\GML\ has a fixed-point number system inherited from \MF and
\MP\footnote{Note that from version 1.800 of \MP there is a
  full-fledged floating point numeric implemented in \MP. This has
  however not yet been implemented into \GML.}.  In this
...
`>> 25`
\end{quote}
The `:=` assignment will remove any previous value, while on the other
hand an `=`
